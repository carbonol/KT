...
for i in range(kc_count):
    print('s_' + str(i + 1) + ' =', pfa_model.get_s()[i])
...
for i in range(kc_count):
    print('f_' + str(i + 1) + ' =', pfa_model.get_f()[i])
print()
...
# 2) Cuando se tiene una probabilidad de 95% de responder un ítem en donde está involucrado una habilidad j cualquiera, 
...
def generate_sucessive_incorrect_answer_graph(pfa_model:pfa.PFA, n_values:list, j:int):
    ...
    m_values = [pfa_model.get_m()[j]]
    p_m_values = [pfa_model.get_p_m()[j]]
    # BORRAR ESTA LÍNEA DESPUÉS DE HACER TESTING:
    print(m_values, p_m_values)
...
# pfa_model = pfa.PFA(kc_count, beta, gamma, rho)
# print(get_sucessive_correct_answers_needed_to_reach_mastery_in_all_skills(pfa_model))